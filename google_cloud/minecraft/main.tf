...
resource "aws_accessanalyzer_analyzer" "accessanalyzer_analyzer1" {
  analyzer_name = 
}
...
resource "aws_acm_certificate_validation" "acm_certificate_validation1" {
  certificate_arn = 
}
...
resource "google_bigquery_table_iam_member" "bigquery_table_iam_member" {
  member = 
  dataset_id = 
  table_id = 
  role = 
